...
echo "2. Input : Job ,trigger file ( testjob.job ,testjob2.job ,test.trg ,test2.trg(embeed in this test script))"
...
echo "** Clean wiperdog var/job and testcase directory before running testcase"
resultJob1=testRemoveTrigger/result/testjob.txt
...
sleep 30
echo "** Remove previous output from job of testjob & testjob2 in $currentDir/testRemoveTrg/result"
rm -rf $resultJob1 > /dev/null
rm -rf $resultJob2 > /dev/null
